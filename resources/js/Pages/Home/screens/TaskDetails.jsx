...
export default function TaskDetails({ taskDetails, boardId, userId, columnDetails, closeModal }) {
    ...
    const updateTask = (e) => {
        e.preventDefault()
        closeModal()
        Inertia.put(`/boards/${boardId}/tasks/${taskDetails.id}`, { title: updatedTitle, priority: updatedPriority, description: updatedDesc }) // Update task with Inertia
    }

    const deleteTask = () => {
        setModal(false)
        closeModal()
        Inertia.delete(`/boards/${boardId}/tasks/${taskDetails.id}`) // Use Inertia to delete the task
    }
