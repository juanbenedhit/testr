...
export default function Kanban({ userId }) {
    ...
    const onDragEnd = (result) => {
        ...
        if (result.type === 'task') {
            ...
            if (startColumn === endColumn) {
                ...
                const newColumn = { ...endColumn, taskIds: newTaskIds }
                const newState = { ...initialData, columns: { ...initialData.columns, [endColumn.id]: newColumn } }
                setInitialData(newState)
                Inertia.put(`/boards/${boardId}/columns/${startColumn.id}`, { taskIds: newTaskIds }) // Update column with Inertia
                return
            }
            ...
            const newFinish = { ...endColumn, taskIds: finishTaskIDs }

            const newState = { ...initialData, columns: { ...initialData.columns, [startColumn.id]: newStart, [endColumn.id]: newFinish } }
            setInitialData(newState)

            Inertia.put(`/boards/${boardId}/columns/${newStart.id}`, { taskIds: startTaskIDs }) // Update start column with Inertia
            Inertia.put(`/boards/${boardId}/columns/${newFinish.id}`, { taskIds: finishTaskIDs }) // Update finish column with Inertia
        } else {
            ...
            Inertia.put(`/boards/${boardId}/columns/columnOrder`, { order: newColumnOrder }) // Update column order with Inertia
        }
    }
    ...
    const addCol = (e) => {
        ...
        Inertia.post(`/boards/${boardId}/columns`, { title: newColumnName }) // Use Inertia to add a new column
        e.target.elements.newCol.value = ''
    }

    const changeBoardName = debounce((ev) => {
        Inertia.put(`/boards/${boardId}`, { name: ev }) // Update board name with Inertia
    }, 7000)
