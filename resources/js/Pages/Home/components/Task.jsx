...
export default function Task({ allData, id, index, boardId, userId, columnDetails, filterBy }) {
    const [modal, setModal] = useState(false)
    const theTask = allData.tasks[id]
    let matched = filterBy === null ? 'all' : theTask.priority === filterBy

    const deleteTask = () => {
        Inertia.delete(`/boards/${boardId}/tasks/${id}`) // Use Inertia to delete the task
    }

    return (
        <div className={`${matched ? '' : 'opacity-10'}`}>
            <Modal modal={modal} setModal={setModal} ariaText='Task Details'>
                <TaskDetails taskDetails={theTask} closeModal={() => setModal(false)} boardId={boardId} userId={userId} columnDetails={columnDetails} deleteTask={deleteTask} />
            </Modal>
                        ...
                        <div className='w-full'>
                            <h4 className='text-sm sm:text-base'>{theTask.title}</h4>
                             <div className='flex mt-2 space-x-3 sm:space-x-5'>
                                ...
                                {theTask.description && theTask.description.length > 1 ? <Description /> : null}
                            </div>
                        </div>
                    </div>
                }
